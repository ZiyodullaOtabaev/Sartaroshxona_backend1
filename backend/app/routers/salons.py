"""
Salons (Sartaroshxona) endpoints — CRM tizimi

Funksiyalar:
- Salon CRUD (yaratish, ko'rish, yangilash)
- Xodimlar boshqaruvi (sartaroshlarni taklif qilish/chiqarish)
- Dinamik taklif/so'rov tizimi (owner <-> barber)
- Owner dashboard: daromad analitikasi (har bir sartarosh + umumiy)
"""
import logging
import datetime
import aiomysql
...
from app.middleware.auth import require_auth, require_owner
from app.schemas.models import (
    SalonCreate, SalonUpdate, StaffInvite, JoinRequest, InvitationResponse,
)
from app.services.helpers import timedelta_to_str, serialize_datetime

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/salons", tags=["Salons (CRM)"])


# ═══════════════════════════════════════════════════════════════════════════════
# HELPER — owner salonini topish va tekshirish
# ═══════════════════════════════════════════════════════════════════════════════

async def _get_owner_salon(cur, user_id: int) -> dict:
    """Owner'ning salonini qaytaradi yoki 404"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# PUBLIC — Salon ko'rish (mijozlar uchun)
# ═══════════════════════════════════════════════════════════════════════════════

@router.get("/{salon_id}")
async def get_salon(salon_id: int):
    """Salon to'liq ma'lumotlari + ishlayotgan sartaroshlar (public)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            ...
            # Salon xodimlari (sartaroshlar)
            await cur.execute(
                "SELECT id, name, specialization, rating, total_reviews, avatar_url, "
                "is_online, is_accepting_bookings, experience "
                "FROM barbers WHERE salon_id=%s",
                (salon_id,),
            )
            ...
            result = serialize_datetime(result, ["created_at", "updated_at"])
...
async def list_salons(
    ...
    """Barcha faol salonlar ro'yxati (public)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            offset = (page - 1) * limit
            await cur.execute(
                "SELECT s.id, s.name, s.description, s.address, s.district, s.lat, s.lng, "
                "s.phone, s.avatar_url, s.cover_url, s.rating, s.total_reviews, "
                "(SELECT COUNT(*) FROM barbers WHERE salon_id=s.id) as barbers_count "
                "FROM salons s WHERE s.is_active=1 "
                "ORDER BY s.rating DESC LIMIT %s OFFSET %s",
                (limit, offset),
            )
...
@router.post("")
async def create_salon(data: SalonCreate, current_user: dict = Depends(require_owner)):
    """Yangi salon yaratish (owner allaqachon salonga ega bo'lsa — xato)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute("SELECT id FROM salons WHERE owner_id=%s", (current_user["user_id"],))
            ...
            await cur.execute(
                ...
                (
                    current_user["user_id"], data.name, data.description, data.address,
                    data.district, data.lat, data.lng, data.phone,
                ),
            )
            await conn.commit()
...
async def update_my_salon(data: SalonUpdate, current_user: dict = Depends(require_owner)):
    """O'z salonimni yangilash"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])

            fields, values = [], []
            for col in ("name", "description", "address", "district", "lat", "lng",
                        "phone", "working_hours_start", "working_hours_end"):
...
@router.get("/me/info")
async def get_my_salon(current_user: dict = Depends(require_owner)):
    """O'z salonim ma'lumotlari"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])
            ...
            result["working_hours_start"] = timedelta_to_str(result.get("working_hours_start"))
            result["working_hours_end"] = timedelta_to_str(result.get("working_hours_end"))
            return serialize_datetime(result, ["created_at", "updated_at"])
    finally:
        await release_conn(conn)


# ═══════════════════════════════════════════════════════════════════════════════
# OWNER — Xodimlar (sartaroshlar) boshqaruvi
# ═══════════════════════════════════════════════════════════════════════════════

@router.get("/me/staff")
async def get_staff(current_user: dict = Depends(require_owner)):
    """Salondagi barcha sartaroshlar + qisqa statistika"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])
            await cur.execute(
                "SELECT b.id, b.name, b.specialization, b.phone, b.rating, b.total_reviews, "
                "b.avatar_url, b.is_online, b.is_accepting_bookings, b.experience, "
...
@router.post("/me/staff/invite")
async def invite_barber(data: StaffInvite, current_user: dict = Depends(require_owner)):
    """Sartaroshni salonga taklif qilish (barber_id yoki email orqali)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])
            ...
            barber = None
            if data.barber_id:
                await cur.execute("SELECT id, salon_id FROM barbers WHERE id=%s", (data.barber_id,))
                barber = await cur.fetchone()
            elif data.barber_email:
                await cur.execute(
                    "SELECT b.id, b.salon_id FROM barbers b "
                    "JOIN users u ON b.user_id=u.id WHERE u.email=%s",
                    (data.barber_email,),
                )
                barber = await cur.fetchone()
            ...
            await cur.execute(
                "INSERT INTO salon_invitations (salon_id, barber_id, initiated_by, message) "
                "VALUES (%s, %s, 'owner', %s)",
                (salon["id"], barber["id"], data.message),
            )
            # Sartaroshga notification
            await cur.execute(
                "SELECT user_id FROM barbers WHERE id=%s", (barber["id"],)
            )
            barber_user = await cur.fetchone()
            if barber_user:
                await cur.execute(
                    "INSERT INTO notifications (user_id, title, body, type) VALUES (%s, %s, %s, 'system')",
                    (barber_user["user_id"], "Yangi taklif!",
                     f"{salon['name']} sizni jamoasiga taklif qilmoqda"),
                )
            await conn.commit()
...
@router.delete("/me/staff/{barber_id}")
async def remove_barber(barber_id: int, current_user: dict = Depends(require_owner)):
    """Sartaroshni salondan chiqarish"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])
            await cur.execute(
                "SELECT id, user_id FROM barbers WHERE id=%s AND salon_id=%s",
                (barber_id, salon["id"]),
            )
            barber = await cur.fetchone()
            if not barber:
...
# ═══════════════════════════════════════════════════════════════════════════════
# INVITATIONS — Taklif/so'rovlar (owner <-> barber)
# ═══════════════════════════════════════════════════════════════════════════════

@router.post("/join-request")
async def request_to_join(data: JoinRequest, current_user: dict = Depends(require_auth)):
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute("SELECT id, salon_id FROM barbers WHERE user_id=%s", (current_user["user_id"],))
            barber = await cur.fetchone()
            if not barber:
...
async def my_invitations(current_user: dict = Depends(require_auth)):
    """Mening taklif/so'rovlarim (barber yoki owner sifatida)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            role = current_user.get("role")
            if role == "barber":
                await cur.execute("SELECT id FROM barbers WHERE user_id=%s", (current_user["user_id"],))
                barber = await cur.fetchone()
                if not barber:
                    return []
                await cur.execute(
                ...
                if not salon:
                    return []
                await cur.execute(
                    "SELECT i.*, b.name as barber_name, b.avatar_url as barber_avatar, "
                    "b.specialization, b.rating "
            ...
            rows = [serialize_datetime(dict(r), ["created_at", "responded_at"]) for r in await cur.fetchall()]
            return rows
    finally:
        await release_conn(conn)
...
async def respond_invitation(
    ...
    """Taklif/so'rovga javob berish (qabul yoki rad)"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute(
                "SELECT i.*, s.owner_id, s.name as salon_name, b.user_id as barber_user_id "
                "FROM salon_invitations i "
                "JOIN salons s ON i.salon_id=s.id "
                "JOIN barbers b ON i.barber_id=b.id "
            ...
            # Kim javob berishi mumkin?
            # owner tashabbus qilgan bo'lsa => barber javob beradi
            # barber tashabbus qilgan bo'lsa => owner javob beradi
            if inv["initiated_by"] == "owner":
                if current_user["user_id"] != inv["barber_user_id"]:
                    raise HTTPException(status_code=403, detail="Bu taklifga javob bera olmaysiz")
            else:
                if current_user["user_id"] != inv["owner_id"]:
            ...
            await cur.execute(
                "UPDATE salon_invitations SET status=%s, responded_at=NOW() WHERE id=%s",
                (new_status, invitation_id),
            )

            if data.accept:
                # Barberni salonga biriktirish
                await cur.execute(
                    "UPDATE barbers SET salon_id=%s WHERE id=%s",
                    (inv["salon_id"], inv["barber_id"]),
                )
                # Boshqa pending takliflarni bekor qilish
                await cur.execute(
                    "UPDATE salon_invitations SET status='cancelled' "
                    "WHERE barber_id=%s AND status='pending' AND id!=%s",
                    (inv["barber_id"], invitation_id),
                )
                # Notification
                await cur.execute(
                    "INSERT INTO notifications (user_id, title, body, type) VALUES (%s, %s, %s, 'system')",
                    (inv["barber_user_id"], "Salonga qo'shildingiz",
                     f"Siz {inv['salon_name']} jamoasiga qo'shildingiz"),
                )
...
# ═══════════════════════════════════════════════════════════════════════════════
# OWNER DASHBOARD — Daromad analitikasi (CRM yuragi)
# ═══════════════════════════════════════════════════════════════════════════════

@router.get("/me/dashboard")
async def owner_dashboard(current_user: dict = Depends(require_owner)):
    """
    Owner bosh paneli:
    - Umumiy daromad (bugun / bu oy / jami)
    - Navbatlar soni (bugun / kutilayotgan / yakunlangan)
    - Har bir sartarosh bo'yicha daromad
    """
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            ...
            await cur.execute(
                ...
                "WHERE b.salon_id=%s AND p.status='completed'",
                (salon_id,),
            )
            ...
            await cur.execute(
                ...
                "JOIN appointments a ON p.appointment_id=a.id "
                "JOIN barbers b ON a.barber_id=b.id "
                "WHERE b.salon_id=%s AND p.status='completed' AND DATE(p.created_at)=%s",
                (salon_id, today),
            )
            ...
            await cur.execute(
                "SELECT "
                "COUNT(CASE WHEN DATE(a.appointment_time)=%s AND a.status!='cancelled' THEN 1 END) as today_count, "
            ...
            return {
                ...
                "appointments": {
                    "today": appt_stats["today_count"],
                    "pending": appt_stats["pending_count"],
                    "completed": appt_stats["completed_count"],
                },
...
@router.get("/me/revenue-report")
async def revenue_report(
    current_user: dict = Depends(require_owner),
    days: int = Query(7, ge=1, le=90),
):
    """Kunlik daromad hisoboti (grafik uchun) — oxirgi N kun"""
    conn = await get_conn()
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            salon = await _get_owner_salon(cur, current_user["user_id"])
